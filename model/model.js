exports.create = function (datasource) {
    ...
    editGeneralInfo : function (info, featureImageTmpPath, callback){
      ...
      addProduct : function(name, price, brand, imageTmpPath, callback){
            if(name == ''){
                callback('Product name connot be empty')
                return
            }
            if(imageTmpPath == ''){
                callback('Product image connot be empty')
                return
            }
            datasource.addProduct(name, price, brand, imageTmpPath, function(err){
                if(err){
